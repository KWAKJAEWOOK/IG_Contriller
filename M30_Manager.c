...
void process_m30_packet(M30_CONTEXT *ctx) {
    // todo. M30 응답 파싱
    // todo. 메시지를 전송했는데 응답이 없는 장치는 connection_status_ptr에 기록해두고, 연결 해제 및 재연결 시도
}
...
const uint8_t packets[10][18] = {   // 밝기 조절 패킷 데이터
...
// ======================= 표출 기능 구현 함수들 ========================
// todo. vms_command_ptr 내부 값이 1일 때, 순차적 애니메이션 표출 / 내부 값이 2일 때, PET 값에 따라 표출 점멸하는 함수 (do_thread 적절히 활용하면 될듯)
// todo. vms_command_ptr 내부의 값과 현재 표출중인 인덱스 비교, 변경된 장치에 적절한 패킷 전송 함수
